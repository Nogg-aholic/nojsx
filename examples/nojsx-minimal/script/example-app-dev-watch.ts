...
async function runBuild(reason: string): Promise<void> {
  ...
  try {
    ...
    console.log(`[nojsx-minimal:watch] rebuilt (${reason})`);
  } catch (error) {
    console.error(`[nojsx-minimal:watch] build failed (${reason})`, error);
  } finally {
    building = false;
    if (buildQueued) {
      buildQueued = false;
...
console.log(`[nojsx-minimal:watch] watching ${appRoot}`);
