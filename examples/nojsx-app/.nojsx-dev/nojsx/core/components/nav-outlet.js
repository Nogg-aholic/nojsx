import { jsx as _jsx } from "nojsx/jsx-runtime";
...
function navInstanceKey(fullPath) {
...
function getPageComponent() {
    ...
    const componentName = picked.entry?.componentName;
    if (!componentName)
        return '';
    const Page = globalThis.__nojsxComponentLoaders?.[componentName];
    if (!Page)
        return '';
...
export class NavOutlet extends NComponent {
    ...
    html = () => _jsx("div", { children: getPageComponent() });
    onLoad = () => {
        globalThis.__nojsxNavOutlet = this;
    };
}
