...
function toRoutePath(fileName: string): string {
  ...
  if (base.toLowerCase() === "home") return "/home";
...
async function discoverPages(pagesRoot: string): Promise<PageModule[]> {
...
async function buildLoaderModule(pagesRoot: string, jsxImportSource: string): Promise<string> {
...
async function readShellPageLayoutFields(shellPagePath: string): Promise<ShellLayoutFields> {
